...
class Dialog {
  constructor (enabled = true) {
    ...
    if (this.enabled) {
      ...
      waitFor('ytd-app', (target) => {
        ;(new MutationObserver(([mutation]) => {
          if (this.dialog && mutation.target) {
            const isWatchPage = mutation.target.hasAttribute('is-watch-page')
            this.dialog.style.display = isWatchPage
              ? 'none'
              : 'block'
          }

        })).observe(target, { attributes: true, attributeFilter: ['is-watch-page'] });
      }, 100)
  ...
  create() {
    ...
    dialog.style.display = this.enabled
      ? 'block'
      : 'none'
...
class Branding {
  constructor(enabled = false) {
    ...
    waitFor('#watch-header, ytd-video-primary-info-renderer', (target) => {
  ...
  _createBranding () {
    ...
    branding.style.display = this.enabled
      ? 'block'
      : 'none'
    return branding
  }
